...
def table_data(data_string):
    data_list = re.split('","|",|,"|L,', data_string)                    # Для products_data_all убрать запятую
...
string_dict = {}
json_list = []
f_string = open(r'F:\Programming\Code\traning\products_data_all.txt', encoding = 'utf-8').read()
string_list = f_string.split('\n')
structure = table_title(string_list[0])
string_list.pop(0)
for string in string_list:
    data_list = table_data(string[:-1])                                         # Для products_data_all добавить [:-1]
    string_count += 1
    ...
    string_dict = {}
json_strings ='[' + ',\n'.join(json_list) + ']'
json_file = open(r'F:\Programming\Code\traning\file.json', 'w', encoding = 'utf-8')
